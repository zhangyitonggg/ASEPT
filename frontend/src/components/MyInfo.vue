  ...
  <v-container>
    <v-card class="mb-10">
      ...
      <v-card-text>
        <v-row class="mb-5 pl-7">
          <v-avatar color="primary" size="128"><img
              src="https://cravatar.cn/avatar/5ed20f2960c5e87468dee55bfd3ec4ab?d=mp">
          </v-avatar>
        </v-row>
        <v-row class="pl-3 pr-3">
          <v-col>
            <v-form @submit.prevent="handleUserModify">
              <v-text-field class="dense" outlined label="用户名" autofocus prepend-inner-icon="mdi-account-box"
                v-model="$store.getters.username" disabled />
              <v-text-field class="dense" outlined label="原密码" type="password" prepend-inner-icon="mdi-fingerprint-off"
                required v-model="original_password" :disabled="loading" />
              <v-text-field class="dense" outlined label="新密码" :rules="[
              ...
              ]" type="password" prepend-inner-icon="mdi-account-key" v-model="password" :disabled="loading" />
              <v-text-field class="dense" outlined label="确认密码" :rules="[v => v == password || '两次密码不一致']"
                type="password" prepend-inner-icon="mdi-account-key-outline" required v-model="confirm_password"
                :disabled="loading" />
              <v-btn block depressed color="primary" type="submit" @click.stop="handleUserModify" :disabled="loading"
                :loading="loading"> 修改 </v-btn>
            </v-form>
            <v-btn block depressed @click.stop="logout" :disabled="loading" class="mt-2" color="error"> 注销 </v-btn>
          </v-col>
        </v-row>
      </v-card-text>
    </v-card>
    <v-card>
      ...
      <v-card-text v-else>
        <v-row class="pl-3 pr-3">
          <v-col>
            <time_status :lastUpdateTime="last_submit_time" :labels="labels" :values="values"
              :total_passed="total_passed" />
          </v-col>
        </v-row>
        <v-row>
          <v-col v-if="choicePassedQuestions + choiceFailedQuestions != 0">
            <user_status :data="[choicePassedQuestions, choiceFailedQuestions]" :labels="['通过次数', '错误次数']"
              :backgroundColor="['#4FCF95FF', '#D30B0BFF']" :title="'选择题统计'"></user_status>
          </v-col>
          <v-col v-if="choicePassedQuestions + blankPassedQuestions + choiceFailedQuestions + blankFailedQuestions != 0">
            <user_status
              ...
              :backgroundColor="['#4FCF95FF', '#4FCF95FF', '#D30B0BFF', '#D30B0BFF']" :title="'总计'"></user_status>
          </v-col>
          <v-col v-if="blankPassedQuestions + blankFailedQuestions != 0">
            <user_status :data="[blankPassedQuestions, blankFailedQuestions]" :labels="['通过次数', '错误次数']"
              :backgroundColor="['#4FCF95FF', '#D30B0BFF']" :title="'填空题统计'"></user_status>
          </v-col>
          <template v-if="choicePassedQuestions + blankPassedQuestions + choiceFailedQuestions + blankFailedQuestions == 0">
            <v-col>
              <v-banner outlined type="info" icon="mdi-gauge-empty">
                <h2>
                  您还没有做过题目。去做点题吧。
                </h2>
              </v-banner>
